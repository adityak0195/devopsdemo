--------------------------------------------------------------
--------------------------------------------------------------
print '-- GetMachineDowntimeUTCData';
--------------------------------------------------------------
--------------------------------------------------------------
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'GetMachineDowntimeUTCData'))
drop FUNCTION GetMachineDowntimeUTCData;
GO
CREATE FUNCTION GetMachineDowntimeUTCData
	(@Machine varchar(255),
...
from (
	...
	union all 
	SELECT top 1 getUTCdate() as  NewStatusTime, StatusType, [Status] as NewStatus,[SubStatus] as NewSubStatus , [StatusTime] as OldStatusTime, [Status] as OldStatus,[SubStatus] as OldSubStatus 
	FROM smartKPIMachineStatusData WHERE [StatusTime]<=@EndTime and [StatusTime]<=getUTCdate() and  Machine=convert(varchar(255), @Machine)  and StatusType=convert(varchar(255), @StatusType )  order by [StatusTime] desc
...
--select * from GetUtilizationOee2Truck1
--	('2019-07-08 00:00:00',
--	60*24,
--	1,
--	'flex',
--	'xxx',
--	'KBLisLaa6MachineThing')
